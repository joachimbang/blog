...
Pour executer un fichier SQL depuis \texttt{pgAdmin} : ouvrir la base \texttt{blog_db},
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Ouvrir une Invite de commandes / PowerShell elevee (executer en tant
        qu'administrateur).
...
\begin{itemize}
    ...
    \item \textbf{Port} : port PostgreSQL (par defaut `5432`)
    ...
    \item \textbf{SSL} : si votre serveur necessite TLS/SSL (souvent `true` en prod)
\end{itemize}
...
router.post('/login', async (req, res) => {
    try {
        ...
        // pour pouvoir le revoquer en cas de deconnexion ou de compromission.
        res.json({
...
\begin{itemize}
    ...
    \item \textbf{GROUP BY b.id, u.nom, u.prenom, u.email} : necessaire parce qu'on utilise une fonction d'aggregation (COUNT). On indique les colonnes non-aggregees qui doivent apparaître dans le SELECT.
    \item \textbf{ORDER BY b.date\_creation DESC} : trie les resultats par date de creation du plus recent au plus ancien.
\end{itemize}
...
Remarques pratiques :
\begin{itemize}
    \item Toujours verifier les colonnes du \texttt{GROUP BY} : toutes les colonnes non-aggregees doivent y apparaitre.
