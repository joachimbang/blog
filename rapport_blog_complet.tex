...
\usepackage{textcomp} % Améliore le support des caractères Unicode textuels
...
% Pour régler le problème du tiret long "—" et de la flèche "↔" globalement :
...
\definecolor{darkred}{RGB}{139,0,0} % <-- AJOUTÉ : Règle l'erreur de couleur manquante !
...
% ============================================================================
% Définition propre du langage CSS absent de listings
...
\lstdefinestyle{javascript}{
    language=Python, % Conservé selon votre choix d'origine
...
\begin{titlepage}
    ...
    \vspace{2cm}
    {\Large\bfseries Joachim BANGIRAHE\par}
\end{titlepage}
...
% (Avant-propos supprimé : page de garde affiche uniquement le nom de l'auteur)
% ============================================================================
...
\begin{itemize}
    ...
    \item \textbf{BLOG\\_TAGS} : table de jonction (relation N:M) qui relie \texttt{BLOGS} et \texttt{TAGS}.
\end{itemize}
...
\subsection{Versioning Semantique (SemVer)}

Le versioning semantique suit le format \texttt{MAJEUR.MINEUR.CORRECTIF} :

\begin{itemize}
    \item \textbf{MAJEUR} : Changements incompatibles avec l'API precedente (ex: 1.0.0 → 2.0.0)
    \item \textbf{MINEUR} : Nouvelles fonctionnalites compatibles (ex: 1.0.0 → 1.1.0)
...
Creez le fichier \texttt{server.js} a la racine du projet. Ce sera le point d'entree de votre application.

\begin{lstlisting}[style=javascript, caption=server.js - Configuration complete du serveur Express]
...
// Importation de la configuration de la base de donnees
...
// Middleware CORS - Permet les requetes cross-origin
app.use(cors());

// Middleware pour parser le JSON dans le corps des requetes
app.use(express.json());

// Middleware pour parser les donnees URL-encoded
...
// Middleware pour logger chaque requete
...
\begin{definitionbox}
    \textbf{import} : Mot-clé ES module pour importer des modules. Dans le systeme de modules moderne de Node.js, \texttt{import} charge un module et permet d'utiliser ses exports nommés ou par défaut.
\end{definitionbox}
...
\section{Schema Complet de la Base de Donnees}

Voici le schema SQL complet de notre base de donnees :

\begin{lstlisting}[style=sql, caption=schema.sql - Schema complet de la base de donnees]
...
Creez le fichier \texttt{db.js} a la racine du projet :
...
const pool = new Pool({
    ...
    idleTimeoutMillis: 30000,   // Temps d'inactivite avant fermeture d'un client
...
pool.on('connect', () => {
    console.log('Nouvelle connexion etablie avec PostgreSQL');
});
...
const testConnection = async () => {
    try {
        ...
        console.log('Connexion reussie a PostgreSQL :', result.rows[0]);
        client.release();
    } catch (err) {
        console.error('Erreur de connexion a PostgreSQL :', err);
        throw err;
    }
};
...
Le projet utilise un schema SQL qui doit etre charge dans la base de donnees
`blog_db` avant de pouvoir enregistrer des utilisateurs et des articles.
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.9\textwidth]{rapport/images/pgadmin_users.png}
    \caption{Capture d'écran de pgAdmin montrant la table \texttt{public.users}}
    \label{fig:pgadmin-users}
\end{figure}
% Placez l'image de la capture d'écran dans le dossier rapport/images/ avec le nom pgadmin_users.png.
...
\begin{errorbox}
    ...
    plutot que d'exécuter directement des commandes dans l'invite Windows.

    Procédure (pgAdmin) :
    \begin{itemize}
        \item Ouvrir \texttt{pgAdmin} et se connecter au serveur en tant que \texttt{postgres}.
        \item Ouvrir le \emph{Query Tool} pour le serveur ou la base ciblée.
        \item Exécuter la commande SQL suivante dans le \emph{Query Tool} :
    \end{itemize}
...
ALTER USER postgres WITH PASSWORD 'nouveau_mot_de_passe';
    ...
    Alternatives en ligne de commande (si souhaité) :
    \begin{lstlisting}[style=bash]
# Linux / macOS :
...
Sur Windows, il est plus simple d'utiliser \texttt{pgAdmin} pour créer une base :
...
Vous pouvez aussi ouvrir le \emph{Query Tool} et exécuter :
...
Pour exécuter un fichier SQL depuis \texttt{pgAdmin} : ouvrir la base \texttt{blog_db},
...
\subsection{Comment savoir / réinitialiser le mot de passe utilisé par pgAdmin}
...
lors de l'installation. Si vous ne vous rappelez plus du mot de passe, procédez
comme suit :

\begin{itemize}
    \item Méthode recommandée (ligne de commande) :
    \begin{itemize}
        \item Ouvrir une Invite de commandes / PowerShell élevée (exécuter en tant
        qu'administrateur).
        \item Appeler le binaire `psql` fourni par PostgreSQL et exécuter une
        commande SQL pour réinitialiser le mot de passe :
    \end{itemize}
    \begin{lstlisting}[style=bash]
"C:\\Program Files\\PostgreSQL\\15\\bin\\psql.exe" -U postgres -c "ALTER USER postgres WITH PASSWORD 'NouveauMotDePasse';"
    \end{lstlisting}
    \item Si vous avez accès à une machine Linux/macOS où PostgreSQL est installé :
    \begin{lstlisting}[style=bash]
sudo -u postgres psql -c "ALTER USER postgres WITH PASSWORD 'NouveauMotDePasse';"
    \end{lstlisting}
    \item Une fois le mot de passe réinitialisé, ouvrez \texttt{pgAdmin} et entrez
    ...
    \item Si vous ne pouvez pas exécuter `psql` localement, vous pouvez démarrer
    PostgreSQL en mode single-user ou utiliser l'installateur pour réparer/réinitialiser,
    mais ces méthodes sont avancées : préférez la commande ci-dessus.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Host} : adresse du serveur (ex. `localhost` ou une IP)
    \item \textbf{Port} : port PostgreSQL (par défaut `5432`)
    \item \textbf{Database} : nom de la base (ex. `blog_db`)
    \item \textbf{User} : nom d'utilisateur PostgreSQL (ex. `postgres`)
    \item \textbf{Password} : mot de passe PostgreSQL (celui réinitialisé ci‑dessus)
    \item \textbf{SSL} : si votre serveur nécessite TLS/SSL (souvent `true` en prod)
\end{itemize}
...
\begin{itemize}
    ...
    \item Vérifiez que PostgreSQL accepte les connexions depuis l'hôte indiqué en
...
Pour des sessions sécurisées, on utilise typiquement des tokens JWT d'accès
(short-lived "access tokens") et des tokens de rafraîchissement (long-lived
"refresh tokens"). Le schéma typique :

\begin{itemize}
        \item L'API émet un `accessToken` (p. ex. 15 minutes) et un `refreshToken`
        ...
        \item Le `refreshToken` peut être révoqué côté serveur lors de la déconnexion.
\end{itemize}

Ci-dessous un exemple minimal en Node.js (Express) montrant l'implémentation
des endpoints `POST /login`, `POST /token` et `POST /logout`, ainsi qu'un
middleware `authenticateToken` pour protéger les routes. Le code complet se
...
// stockage mémoire des refresh tokens (pour démo). En prod : stocker en DB.
let refreshTokens = [];

app.post('/login', async (req, res) => {
    // vérifier utilisateur / mot de passe (hash)
...
Le code d'exemple complet est fourni dans les fichiers créés : `server.js`,
...
Creez le fichier \texttt{routes/auth.js} :
...
router.post('/login', async (req, res) => {
    try {
        ...
        // En production, stockez le refresh token dans la base de donnees ou un cache
        // pour pouvoir le révoquer en cas de deconnexion ou de compromission.
        res.json({
...
router.post('/logout', (req, res) => {
    // En production, invalidez le refresh token dans la base ou supprimez-le du store.
...
\subsection{Route d'Inscription}

\begin{lstlisting}[style=javascript, caption=POST /api/auth/register]
router.post('/register', async (req, res) => {
    // Validation des donnees
    const { email, password, nom, prenom } = req.body;
...
\subsection{Route de Connexion}

\begin{lstlisting}[style=javascript, caption=POST /api/auth/login]
router.post('/login', async (req, res) => {
    const { email, password } = req.body;
    ...
    // Generation des tokens d'acces et de rafraichissement
    const accessToken = jwt.sign(
...
Creez le fichier \texttt{routes/blogs.js} :

\begin{lstlisting}[style=javascript, caption=routes/blogs.js - Routes des blogs]
import express from 'express';
import db from '../db.js';
import authMiddleware from '../middleware/auth.js';
import { checkRole } from '../middleware/roles.js';
...
\subsection{Explication pas-a-pas de la requete}

La requete ci-dessus combine plusieurs concepts SQL courants. Nous expliquons chacun et donnons de petits exemples.

\begin{itemize}
    \item \textbf{SELECT b.* , ...} : selectionne toutes les colonnes de la table \texttt{blogs} (alias \texttt{b}) et ajoute des colonnes calculees ou provenant d'autres tables.
    \item \textbf{LEFT JOIN users u ON b.auteur\_id = u.id} : joint la table \texttt{users} a \texttt{blogs}. LEFT JOIN signifie "garder toutes les lignes de la table de gauche (blogs)" meme si aucune correspondance n'existe dans \texttt{users} (les colonnes de \texttt{users} seront NULL si pas de match).
    \item \textbf{LEFT JOIN commentaires c ON b.id = c.blog\_id} : joint les commentaires associes a chaque blog. Un blog sans commentaires reste presente avec NULLs pour \texttt{c}.
    \item \textbf{COUNT(c.id) as nombre\_commentaires} : compte le nombre de lignes de la jointure pour chaque blog (nombre de commentaires). COUNT(ignore les NULLs quand on compte une colonne specifique).
    \item \textbf{GROUP BY b.id, u.nom, u.prenom, u.email} : necessaire parce qu'on utilise une fonction d'aggregation (COUNT). On indique les colonnes non-aggregées qui doivent apparaître dans le SELECT.
    \item \textbf{ORDER BY b.date\_creation DESC} : trie les resultats par date de creation du plus récent au plus ancien.
\end{itemize}

\paragraph{Petit exemple illustratif}

Supposons ces petites tables :
\begin{lstlisting}[style=sql]
blogs:            users:           commentaires:
id | titre        id | nom         id | blog_id
1  | "Post A"     10 | "Alice"    100 | 1
2  | "Post B"     11 | "Bob"      101 | 1
                                102 | 2
\end{lstlisting}

Si on fait un \texttt{LEFT JOIN} de \texttt{blogs} avec \texttt{commentaires}, on obtient :
\begin{lstlisting}[style=sql]
SELECT b.id, b.titre, c.id as commentaire_id
FROM blogs b
LEFT JOIN commentaires c ON b.id = c.blog_id;
...
Appliquer \texttt{COUNT(c.id)} avec \texttt{GROUP BY b.id} donne le nombre de commentaires par blog :
\begin{lstlisting}[style=sql]
SELECT b.id, b.titre, COUNT(c.id) as nb
FROM blogs b
LEFT JOIN commentaires c ON b.id = c.blog_id
...
Remarques pratiques :
\begin{itemize}
    \item Toujours verifier les colonnes du \texttt{GROUP BY} : toutes les colonnes non-aggregées doivent y apparaitre.
    \item Utiliser \texttt{LEFT JOIN} si vous voulez garder les entites de gauche meme si elles n'ont pas de correspondance.
    \item Si vous utilisez \texttt{JOIN} (INNER JOIN), les lignes sans correspondance seront exclues.
\end{itemize}
...
router.put('/:id', authMiddleware, checkRole('auteur', 'admin'), async (req, res) => {
    try {
        ...
        // Verification que le blog existe
        ...
        // Verification des droits (auteur ou admin)
...
Creez le fichier \texttt{routes/commentaires.js} :

\begin{lstlisting}[style=javascript, caption=routes/commentaires.js - Routes des commentaires]
import express from 'express';
import db from '../db.js';
import authMiddleware from '../middleware/auth.js';
...
<head>
    ...
    <title>Blog Multi-Role</title>
    <link rel="stylesheet" href="css/style.css">
</head>
<body>
    <header>
        <h1>Blog Multi-Role</h1>
        <nav>
            <a href="index.html">Accueil</a>
            <a href="login.html" id="loginLink">Connexion</a>
            <a href="register.html" id="registerLink">Inscription</a>
            <a href="#" id="logoutLink" style="display: none;">Deconnexion</a>
        </nav>
    </header>
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Connexion - Blog Multi-Role</title>
    <link rel="stylesheet" href="css/style.css">
</head>
<body>
    <div class="container">
...
\begin{lstlisting}[style=javascript, caption=public/js/app.js - JavaScript principal]
...
async function loadBlogs() {
    try {
        let response = await fetch(`${API_URL}/blogs`, {
        ...
        data.blogs.forEach(blog => {
            ...
            blogElement.innerHTML = `
                <h3>${blog.titre}</h3>
                <p class="author">Par ${blog.nom} ${blog.prenom}</p>
                <p class="date">${new Date(blog.date_creation).toLocaleDateString('fr-FR')}</p>
                <p>${blog.contenu.substring(0, 200)}...</p>
                <a href="blog.html?id=${blog.id}">Lire la suite</a>
            `;
...
\begin{lstlisting}[style=javascript, caption=public/js/login.js - JavaScript de connexion]
const API_URL = 'http://localhost:3000/api';
...
\begin{lstlisting}[style=css, caption=public/css/style.css - Styles CSS]
* {
    margin: 0;
    padding: 0;
...
Ce projet pedagogique nous a permis de developper une application web complete de blog multi-role en utilisant les technologies modernes suivantes :
